...
func SecureSignupController(ctx *gin.Context) {
	var requestData model.RequestModel
	ctx.ShouldBindBodyWithJSON(&requestData)
	domainval, err := helper.IfEmailIsAllowed(requestData.UserEmail)
	if err != nil {
...
func SecureVerificationController(ctx *gin.Context) {
	var requestData model.SignUpResponse
	ctx.ShouldBindBodyWithJSON(&requestData)
	response, err := helper.SecureVerifyCode(requestData.UserId, requestData.Vcode)
	if err != nil {
		fmt.Println(err)
		ctx.String(http.StatusBadRequest, "invalid")
		// ctx.JSON(http.StatusBadRequest, "invalid")
...
func SecureLoginSendmailContoller(ctx *gin.Context) {
	var requestData model.RequestModel
	ctx.ShouldBindBodyWithJSON(&requestData)
	domainval, err := helper.IfEmailIsAllowed(requestData.UserEmail)
	if err != nil {
		...
		response, err := helper.SecureLoginSendEmail(domainval, requestData)
		if err != nil {
			ctx.String(http.StatusBadRequest, err.Error())
		} else {
			ctx.JSON(http.StatusAccepted, response)
...
func SecureLoginVerifyController(ctx *gin.Context) {
	var requestData model.SignUpResponse
	ctx.ShouldBindBodyWithJSON(&requestData)
	response, err := helper.SecureLoginVerifyCode(requestData.UserId, requestData.Vcode)
	if err != nil {
		fmt.Println(err)
		ctx.String(http.StatusBadRequest, "invalid")
		// ctx.JSON(http.StatusBadRequest, "invalid")
